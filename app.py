...
# CSS para separar secciones sin usar matplotlib
st.markdown("""
    <style>
    .section-card {
...
try:
    ...
    # --- SECCIÓN 1: CARTERA (CÁLCULO PURO USD) ---
    ...
    if not df_actual.empty:
        ...
        with st.spinner('Sincronizando con Wall Street...'):
            ...
            # Valuación = (CEDEARs / Ratio) * Precio Acción EEUU
            ...
            for _, row in df_actual.iterrows():
                ...
                if not m_t.empty:
                    # Buscamos precio USD de la fecha de compra en Yahoo Finance
                    h_p = yf.download(fix_ticker(t), start=m_t['Fecha'].min(), progress=False)['Close']
                    costo_acum_usd = 0
                    
                    for _, op in m_t.iterrows():
                        try:
                            # Precio en Wall Street el día de la compra
                            p_hist_usd = h_p.loc[:op['Fecha']].ffill().iloc[-1]
                            if isinstance(p_hist_usd, pd.Series): p_hist_usd = p_hist_usd.iloc[0]
                            # Costo en USD = (Cantidad / Ratio) * Precio USD Histórico
                            costo_acum_usd += (op['Cantidad'] / op['Ratio']) * p_hist_usd
                        except:
                            costo_acum_usd += (op['Cantidad'] / op['Ratio']) * row['Precio Hoy']
                    
                    gan_nom = row['Valuación USD'] - costo_acum_usd
                    ret_p = (gan_nom / costo_acum_usd * 100) if costo_acum_usd > 0 else 0
                    ganancias_usd.append(gan_nom)
                    retornos_perc.append(ret_p)
                else:
                    ganancias_usd.append(0)
                    retornos_perc.append(0)
        ...
        with c1:
            ...
            sel_port = st.dataframe(
                df_actual[['Ticker_EEUU', 'Cantidad', 'Ratio', 'Valuación USD', 'Precio Hoy', 'Ganancia (USD)', 'Retorno (%)']],
    ...
    if 'sel_port' in locals() and len(sel_port.selection.rows) > 0:
        ...
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.subheader(f"🔍 Historial USD: {t_sel}")
    ...
    # --- SECCIÓN 3: EVOLUCIÓN (ESTÁTICA) ---
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.subheader("📈 Evolución Global (USD)")
